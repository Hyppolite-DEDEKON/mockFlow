...
export function PlaceholderScreen({ variant = "phone" }: PlaceholderScreenProps) {
  ...
  return (
    ...
    >
      <div
        className={`absolute top-16 left-4 right-4 bg-white/10 animate-pulse ${
          isTablet ? "h-40 rounded-xl left-6 right-6 top-16" : isSamsung ? "h-28 rounded-sm" : "h-32 rounded-xl"
        }`}
      />
      <div
      ...
      <div className="absolute inset-0 flex items-center justify-center bg-black/20">
        ...
        >
          <Play size={isTablet ? 28 : 20} className={`text-white ${isTablet ? "ml-2" : "ml-1"}`} />
        </div>
      </div>
    </div>
  );
